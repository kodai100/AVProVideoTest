    ...
    public class OutputDemo : MonoBehaviour
    {
        ...
        public void OnGUI()
        {
            ...
            if (_decklink.Device != null && _decklink.Device.IsStreamingOutput)
            {
				...
				GUILayout.Label(string.Format("Dropped or Late: {0:D2}", _decklink.Device.GetDroppedOutputFrames()));
				GUILayout.Label(string.Format("Health: {0:000.00}%", (_decklink.Device.HealthStatus*100.0f)));
        ...
        private void EnumerateDevices()
        {
            ...
            for (int i = 0; i < numDevices; i++)
            {
                ...
                for (int j = 0; j < device.NumOutputModes; j++)
                {
                    ...
                    print("  mode " + j + ": " + mode.Width + "x" + mode.Height + " @" + mode.FrameRate.ToString("F2") + "fps [" + mode.PixelFormatDescription + "] idx:" + mode.Index + " " + (mode.InterlacedFieldMode ? "Interlaced" : "Progressive") + " " + (mode.SupportStereo3D ? "(3D)" : ""));
                }
            }
        }
    }
