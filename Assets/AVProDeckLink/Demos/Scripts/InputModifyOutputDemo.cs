...
namespace RenderHeads.Media.AVProDeckLink.Demos
{
    public class InputModifyOutputDemo : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // Space to toggle GUI (improves performance)
			if (Input.GetKeyDown(KeyCode.Space))
			{
				_showGui = !_showGui;
			}
            // Hold space to clear frames
            else if (Input.GetKey(KeyCode.Return))
            {
                if (_outputDecklink._camera.targetTexture != null)
                {
                    RenderTexture.active = _outputDecklink._camera.targetTexture;
		...
		public void OnGUI()
		{
			...
			if (GUILayout.Button("Press SPACE to hide/show this debug UI (improves performance)", GUILayout.ExpandWidth(false)))
			{
			    _showGui = !_showGui;
			}
			GUILayout.EndArea();
            ...
            {
                if (_inputDecklink.Device != null && _inputDecklink.Device.IsStreaming)
                {
                    int totalBufferCount, readBufferCount, usedFrameCount, pendingFrameCount;
                    if (_inputDecklink.Device.GetInputBufferStats(out totalBufferCount, out readBufferCount, out usedFrameCount, out pendingFrameCount))
                    {
                        GUILayout.Label(string.Format("Input Buffer {0}/{1}", readBufferCount, totalBufferCount));
                        GUILayout.Label(string.Format("Input Buffer Full {1} Empty {0}", usedFrameCount, pendingFrameCount));
                    }
                }
                ...
                GUILayout.Label(string.Format("App {0} d {1} cap {2}", Application.targetFrameRate, Time.deltaTime.ToString("F2"), Time.captureFramerate));
				...
				if (_inputDecklink.Device != null)
				{
					GUILayout.Label("Dropped - Input: " + _inputDecklink.Device.GetDroppedInputFrames() + " Output: " + _outputDecklink.Device.GetDroppedOutputFrames());
				}
