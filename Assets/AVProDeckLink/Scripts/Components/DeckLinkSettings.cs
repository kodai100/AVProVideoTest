...
namespace RenderHeads.Media.AVProDeckLink
{
	...
	public class DeckLinkSettings : Singleton<DeckLinkSettings>
	{
		...
		// RJT NOTE: Used to be manual but have since automated to avoid confusion/unecessary performance issues
		// - See 'DeckLinkManager::LateUpdate()'
		[HideInInspector]
		...
		private List<Device> findDevices(int deviceIndex, string deviceName, bool single)
		{
			...
			{
				filtered.Add(DeckLinkManager.Instance.GetDeviceByIndex(i));
			}
		...
		void Start()
		{
			...
			// RJT NOTE: Now automated so forcing to false to avoid conflicting serialised value
			_multiOutput = false;
		}
	}
}
